...
@push('local.scripts')
    function __form(__, obj)
    {
        if (obj.status == 'ok')
        {
            ...
            {
                M.toast({ html: 'İndirim Günü Güncellendi', classes: 'green' })
            }
        }
    }
    ...
    @if (session('status') == 'created')
        M.toast({ html: 'İndirim Günü Oluşturuldu', classes: 'green' })
    @endif
@endpush
    ...
    <form method="{{ @$day ? 'patch' : 'put' }}" action="{{ route('admin.discount.day') }}" class="json" id="details-form" data-callback="__form">
        ...
        <div class="card">
            <div class="card-image">
                <img src="{{ asset('img/md-s/19.jpg') }}" alt="{{ @$day ? 'İndirim Günü' : 'İndirim Günü Oluştur' }}" />
