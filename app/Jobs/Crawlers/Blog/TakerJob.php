...
class TakerJob implements ShouldQueue
{
    ...
    {
        $crawler = BlogCrawler::where('id', $this->data['site_id'])->first();
            ...
            else if ($item->status == 'err' || $item->status == 'failed')
            {
                ...
                if ($crawler->error_count >= $crawler->off_limit)
                {
                    System::log(
                        ...
                        'App\Jobs\Crawlers\Blog\TakerJob::handle(int '.$crawler->id.')',
                        10
                    );

                    if ($crawler->status == true)
                    {
                        Mail::queue(new ServerAlertMail($crawler->name.' Blog Botu [DURDU]', json_encode($item->error_reasons)));
                    }
        ...
        {
            System::log(
                'Blog botu TakerJob tarafından bulunamadı.',
                'App\Jobs\Crawlers\Blog\TakerJob::handle(int '.$this->data['site_id'].')',
                10
            );
        }
    }
